...
6. 沙箱迁移准备（本文件；迁移包经评估后取消——见第五节）
7. 解混淆参考实现并入 tools/re-analysis/（供系统改版时复用）
...
- 本沙箱（proot）可整体删除：**所有有价值内容已归档 GitHub**（代码、文档、逆向参考），无本地迁移包
- 迁移包经评估取消：原拟打包的敏感文件（GitHub 令牌、打卡系统分析存档）与截图均无保留价值
  （令牌可重新认证、存档含隐私且结论已入文档、截图为过程记录），将随沙箱一并清理
