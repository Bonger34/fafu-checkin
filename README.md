...
A：基本不会。保活只是轻量接口请求；仅当登录状态失效时才会刷新：
- 熄屏下为静默刷新（屏幕不亮、不唤醒，无感知）；
- 亮屏下会短暂出现打卡页数秒后自动关闭（仅异常时发生，且有 30 分钟冷却）。

**Q：亮屏时保活还工作吗？**

A：工作。保活调用不受屏幕状态影响（每 15 分钟一次）；只有“调用失败后的刷新”方式因屏幕状态不同
（熄屏静默 / 亮屏短暂弹页），两者都会及时修复会话，避免失效拖延。
...
- 查看日志，每次保活都有一条记录且带具体 token：
  ...
  - 失败：`保活: ❌ 调用失败 2_... (今日 24 成功 / 1 失败) — 屏幕亮着，稍后再试`
